...
pub fn ldx_hu(insn: AssemblyInstruction, irs: &mut Vec<Ir>) {
    ...
    let ir = Ir::from(None, IrOpcode::LOAD, ir_operand16.clone(), ir_operand1.clone(), IrOperand::None);
    irs.push(ir);

    let ir = Ir::from(None, IrOpcode::ZeroExtend(64, 16), ir_operand1, ir_operand16, IrOperand::None);
    irs.push(ir);
}
