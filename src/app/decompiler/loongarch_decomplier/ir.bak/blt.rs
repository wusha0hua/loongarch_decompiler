...
pub fn blt(insn: AssemblyInstruction, irs: &mut Vec<Ir>) {
    let mut operand1 = ir::CondOeprand::None;
    if let Some(operand) = insn.operand1 {
        operand1 = CondOeprand::Reg(ir::Register::from(Register::GR(operand.value), 64, Some(true)));
    } 

    let mut operand2 = ir::CondOeprand::None;
    if let Some(operand) = insn.operand2 {
        operand2 = CondOeprand::Reg(ir::Register::from(Register::GR(operand.value), 64, Some(true)));
    }
