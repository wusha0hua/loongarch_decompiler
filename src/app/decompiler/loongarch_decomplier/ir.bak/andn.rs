...
pub fn andn(insn: AssemblyInstruction, irs: &mut Vec<Ir>) {
    ...
    let ir = Ir::from(Some(insn.address), IrOpcode::NOT, ir_operand3.clone(), ir_operand3.clone(), IrOperand::None);
    irs.push(ir);
    
    let ir = Ir::from(None, IrOpcode::AND, ir_operand1, ir_operand2, ir_operand3);
    irs.push(ir);
}
