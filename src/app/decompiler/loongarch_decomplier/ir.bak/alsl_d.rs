...
pub fn alsl_d(insn: AssemblyInstruction, irs: &mut Vec<Ir>) {
    ...
    if let Some(operand) = insn.operand2 {
        ir_operand = IrOperand::Reg(ir::Register::from(Register::GR(operand.value), 64, None));
    }

    let ir = Ir::from(address, opcode, ir_operand.clone(), ir_operand.clone(), IrOperand::Imm(ir::Immediate::from(sa as isize + 1, 64, false)));
    irs.push(ir);
    ...
    let ir = Ir::from(None, IrOpcode::ADD, ir_operand64, ir_operand, ir_operand3);
    irs.push(ir);
