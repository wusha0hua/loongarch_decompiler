...
pub fn lu32i_d(insn: AssemblyInstruction, irs: &mut Vec<Ir>) {
    ...
    let ir = Ir::from(Some(insn.address), IrOpcode::AND, ir_operand1.clone(), ir_operand1.clone(), IrOperand::Imm(ir::Immediate::from(((1 << 32) - 1), 64, false)));
    irs.push(ir);

    let mut ir_operand2 = IrOperand::None;
    if let Some(operand) = insn.operand2 {
        ir_operand2 = IrOperand::Imm(ir::Immediate::from((operand.value as isize) << 32, 64, true));
    }
