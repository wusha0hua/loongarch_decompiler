...
pub fn slti(insn: AssemblyInstruction, irs: &mut Vec<Ir>) {
    ...
    if let Some(operand) = insn.operand3 {
        ir_operand3 = IrOperand::Imm(ir::Immediate::from(operand.value as isize, 64, true));
    }

    let ir = Ir::from(Some(insn.address), IrOpcode::SLT, ir_operand1, ir_operand2, ir_operand3);
    irs.push(ir);
}
