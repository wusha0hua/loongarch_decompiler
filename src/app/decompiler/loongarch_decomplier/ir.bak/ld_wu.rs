...
pub fn ld_wu(insn: AssemblyInstruction, irs: &mut Vec<Ir>) {
    ...
    let ir = Ir::from(None, IrOpcode::LOAD, ir_operand32.clone(), ir_operand2, IrOperand::None);
    irs.push(ir);

    let ir = Ir::from(None, IrOpcode::ZeroExtend(64, 32), ir_operand1, ir_operand32, IrOperand::None);
    irs.push(ir);
}
