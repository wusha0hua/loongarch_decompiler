...
pub fn nor(insn: AssemblyInstruction, irs: &mut Vec<Ir>) {
    ...
    let ir = Ir::from(Some(insn.address), IrOpcode::OR, ir_operand1.clone(), ir_operand2, ir_operand3);
    irs.push(ir);

    let ir = Ir::from(None, IrOpcode::NOT, ir_operand1.clone(), ir_operand1, IrOperand::None);
    irs.push(ir);
}
