...
pub fn bnez(insn: AssemblyInstruction, irs: &mut Vec<DataFlowIr>, gr_states: &mut Vec<GRRecord>, symbol_table: &mut DFISymbolRecordTable, gr_states_parameter: &mut Vec<GRRecord>, symbol_parameter: &mut HashSet<DFISymbolRecord>) {
    ...
    let address = insn.address as i64 + (operand2.value << 2) as i64;
    let index1 = operand1.value as usize;
    let index2 = operand2.value as usize;

    let mut ir = DataFlowIr {
        address: insn.address,
        opcode: DataFlowIrOpcode::Jcc(Relation::NE),
        operand1:None,
