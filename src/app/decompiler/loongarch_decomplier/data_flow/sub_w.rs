...
pub fn sub_w(insn: AssemblyInstruction, irs: &mut Vec<DataFlowIr>, gr_states: &mut Vec<GRRecord>, symbol_table: &mut DFISymbolRecordTable, gr_states_parameter: &mut Vec<GRRecord>, symbol_parameter: &mut HashSet<DFISymbolRecord>) {
    let mut nop = true;
    ...
    if let Some(symbol) = sym1 {
        gr_states[index1].state = true;
        gr_states[index1].value = RegisterRecord::Symbol(symbol);
