...
pub fn ld_w(insn: AssemblyInstruction, irs: &mut Vec<DataFlowIr>, gr_states: &mut Vec<GRRecord>, symbol_table: &mut DFISymbolRecordTable, gr_states_parameter: &mut Vec<GRRecord>, symbol_parameter: &mut HashSet<DFISymbolRecord>) {
    let mut nop = true;
        ...
        RegisterRecord::Symbol(symbol) => {
            match &symbol.address {
                Address::Stack(stack) => {
                    ...
                    new_symbol.address = Address::Stack(stack + value);
                    new_symbol.value = true;
                    new_symbol.size = Size::Signed32;
                    ...
                    gr_states[index1].state = true;
                    gr_states[index1].value = RegisterRecord::Symbol(new_symbol); 
