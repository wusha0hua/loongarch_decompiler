...
pub fn slli_d(insn: AssemblyInstruction, irs: &mut Vec<DataFlowIr>, gr_states: &mut Vec<GRRecord>, symbol_table: &mut DFISymbolRecordTable, gr_states_parameter: &mut Vec<GRRecord>, symbol_parameter: &mut HashSet<DFISymbolRecord>) {
    let mut nop = true;
    ...
    if value == 0 {
        ...
        match &gr_states[index2].value {
            RegisterRecord::Number(number) => gr_states[index1].value = RegisterRecord::Number(Number::from(number.value, true, Size::Signed64)),
            RegisterRecord::Symbol(symbol) => gr_states[index1].value = RegisterRecord::Symbol(DFISymbolRecord {
        ...
        match &gr_states[index2].value {
            ...
            RegisterRecord::Symbol(symbol2) => {
                ...
                let ir = DataFlowIr {
                    ...
                    operand3: Some(DFIOperand::Number(Number::from(1 << value, true, Size::Signed64))),
                };
